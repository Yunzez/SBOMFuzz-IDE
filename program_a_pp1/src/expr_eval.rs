    ...
    match op {
        ...
        '/' => {
            if rhs == 0 {
                ...
                lhs.checked_div(rhs).ok_or("Division overflow")
            }
        },
        '^' => {
            let exp = u32::try_from(rhs).map_err(|_| "Power RHS must be positive")?;
            Ok(lhs.saturating_pow(exp))
        },
        '%' => {
            if rhs == 0 {
                Err("Modulus RHS must not be zero")
            } else {
                lhs.checked_rem(rhs).ok_or("Modulus error")
            }
        },
