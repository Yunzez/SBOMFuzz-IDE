...
fuzz_target!(|data: &[u8]| {
    // Need at least 3 bytes: two for index, the rest for the number
    if data.len() < 3 {
        return;
    }

    // Map first two bytes to a country index
    let idx = (u16::from_le_bytes([data[0], data[1]]) as usize) % COUNTRIES.len();
    let default_region = Id::from_str(COUNTRIES[idx]).ok();

    // Remaining bytes as the phone number
    if let Ok(number) = std::str::from_utf8(&data[2..]) {
        let _ = parse(default_region, number);
    }
});
